...
Flags (1 byte):
: The current value is 0b11010000, representing optional, extended-length, transitive.

Type (1 byte):
: The current value is TBD, which is waiting for the IANA assignment.

FCList Length (2 bytes):
: The value is the total length of the FCList in bytes.

FCList (variable length):
: The value is a sequence of FCs, in order. The definition of the FC format is shown in {{figure2}}.
...
: 4 bytes, the AS number of the previous AS. If the current AS has no previous AS hop, it MUST be filled with 0.

Current Autonomous System Number (CASN):
: 4 bytes, the AS number of the current AS.

Nexthop Autonomous System Number (NASN):
: 4 bytes, the AS number of the next hop AS to whom the current AS will send the BGP UPDATE message.
...
Signature Length:
: 2 bytes, indicating the signature length in bytes.

Signature:
: variable length, the signature content, and order are Signature=ECDSA(SHA256(PASN, CASN, NASN, Prefix)), where the Prefix is the IP address prefix which is encapsulated in the BGP UPDATE, and only one prefix is used each time.
